...
export async function ebsiCli(...commands) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const processBuffer = () => {
      if (!stdoutBuffer.endsWith("==> \x1b[5G")) return;
    ...
    const next = () => {
      if (commands.length === 0) succeed();
      else child.stdin.write(`${commands.shift()}\n`);
    };
    ...
    child.on("close", (code) => {
      ...
      fail(
        new Error(
          `EBSI CLI terminated before all responses were collected (code ${code ?? "null"}).`,
        ),
      );
    });
